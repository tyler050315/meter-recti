...
async function startNativeBarcodeScanner() {
  ...
  if (!scanner?.scanBarcode) {
    calibrationMessage.textContent = "原生扫码插件未加载，请确认已安装并同步 @capacitor/barcode-scanner。";
    calibrationBadge.className = "badge error";
    calibrationBadge.textContent = "插件未加载";
    return;
  }

  calibrationMessage.textContent = "正在启动原生扫码...";
  calibrationBadge.className = "badge";
  ...
  try {
    ...
    if (!scannedText) {
      calibrationMessage.textContent = "未识别到扫码内容。";
      calibrationBadge.className = "badge";
      calibrationBadge.textContent = "待机";
      return;
    }

    serialNumberInput.value = scannedText;
    calibrationMessage.textContent = "扫码成功，序列号已填入。";
    calibrationBadge.className = "badge connected";
    calibrationBadge.textContent = "扫码成功";
  } catch (error) {
    calibrationMessage.textContent = `原生扫码失败：${error?.message || error || "未知错误"}`;
    calibrationBadge.className = "badge error";
